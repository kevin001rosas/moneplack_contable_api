...
{
    public class clasificacionesDeProductoController : ApiController
    {
        public string getCombobox()
        {
            /*if (!utilidades.validar_token(Request))
                return Json("incorrecto");*/
            string query = "SELECT id, nombre from cf_clasificaciones_de_producto where estado=1 order by nombre;";
        ...
        public string getSearchByPage()
        {
            ...
            ", a.nombre " +
            "from cf_clasificaciones_de_producto a " +
            "where a.estado=1   " +
        ...
        public string Post(int id, [FromBody]Object value)
        {
            ...
            string update_query = string.Format("UPDATE `cf_clasificaciones_de_producto` " +
             "set " +
            "nombre = '{0}' " +
            "where id='{1}' "
        ...
        public string Post([FromBody]Object value)
        {
            try
            {
                ...
                //Actualizamos los datos con un update query. 
                string insert_query = string.Format("INSERT INTO `cf_clasificaciones_de_producto` " +
                "(`nombre`) " +
                "VALUES " +
                "('{0}');"
                    , json["nombre"].ToString().Replace("'", "''"));
